...
export class AuthController {
  ...
  async signIn(@Body() signInDto: SignInDto, @Request() req, @Response() res) {
    ...
    const userLogin = await this.authService.signIn(signInDto, accessTokenCookie)
    let { currentHashedRefreshToken, createdUser,
      updatedDatetime, createdDatetime,
      password, tokenActiveEmail, __v,
      updatedUser, ...cloneUserLogin } = userLogin;
    res.json({ loginStatus: "success", user: cloneUserLogin });
    // return this.authService.signIn(signInDto, accessTokenCookie)
  }
